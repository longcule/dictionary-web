...
import os, random, json, shutil, datetime
 ...
 # Đường dẫn đến file CSV
    ...
    if src_img is not None:
        for items in src_img:
            ...
            link_img = f"https://logcue.duckdns.org/image/{file_name}.png"
...
async def create_user(request: Request, src_img: UploadFile = File(None)):
    ...
    link_img = "https://logcue.duckdns.org/image/avatar.png"
    path_to_image = f"/var/www/build/image/{new_id}.png"
    if src_img is not None:
        with open(path_to_image, "wb") as image:
            image.write(src_img.file.read())
        link_img = f"https://logcue.duckdns.org/image/{new_id}.png"
